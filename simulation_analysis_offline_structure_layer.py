...
if __name__ == '__main__':
  ...
  second_index = 0
  indices = np.arange(0, 8, dtype=int)
  N_skip_stresslet = 0
  N_skip = 1
  N_hist = 4
  number_simulation = 2121
  N_samples = 10
  rcut = 0.11
  z_profile = False
  gr_blobs = False
  ...
  for k, i in enumerate(indices):  
    ...
    if z_profile:
      print('z-profile')
      name = file_prefix + '.' + str(i) + '.' + str(second_index) + '.base.histogram.z'
      value = np.ones((x.shape[0], x.shape[1], 1))
      hz = sa.compute_histogram_from_frames(x, value=None, column_sample=2, column_value=0, num_intervales=40, xmin=0, xmax=wall_Lz, N_avg=N_avg, file_prefix=name)    
      hz_mean = np.zeros((hz.shape[1], 3))
      hz_mean[:,0] = hz[0,:,0]
      for j in range(N_skip, N_hist):
        hz_mean[:,2] += (j - N_skip) * (hz[j,:,1] - hz_mean[:,1])**2 / (j - N_skip + 1)
        hz_mean[:,1] += (hz[j,:,1] - hz_mean[:,1]) / (j - N_skip + 1)
      hz_mean[:,2] = np.sqrt(hz_mean[:,2] / np.maximum(1, N_hist - N_skip) / np.maximum(1, N_hist - N_skip - 1))
      name = file_prefix + '.' + str(i) + '.' + str(second_index) + '.base.histogram.z.dat'
      np.savetxt(name, hz_mean, header='Columns: z, h(z), error h(z)')
      
    # g(r) blobs
    if gr_blobs:
      print('g(r) blobs')
      gr_mean = np.zeros((100, 3))
      for j in range(N_skip, N_hist):
        name = file_prefix + '.' + str(i) + '.' + str(second_index) + '.base.gr.blobs.' + str(j) + '.dat'
        gr = sa.radial_distribution_function(x[j * N_avg : (j+1) * N_avg], N_avg, rcut=0.5, nbins=100, r_vectors=r_vectors, L=L, dim='q2d', name=name)      
        gr_mean[:,0] = gr[:,0]
        gr_mean[:,2] += (j - N_skip) * (gr[:,1] - gr_mean[:,1])**2 / (j - N_skip + 1)
        gr_mean[:,1] += (gr[:,1] - gr_mean[:,1]) / (j - N_skip + 1)
      gr_mean[:,2] = np.sqrt(gr_mean[:,2] / np.maximum(1, N_hist - N_skip) / np.maximum(1, N_hist - N_skip - 1))
      name = file_prefix + '.' + str(i) + '.' + str(second_index) + '.base.gr.blobs.dat'
      np.savetxt(name, gr_mean, header='Column: r, g(r), error g(r)')
    ...
    if gr_bodies:
      ...
      gr_mean = np.zeros((100, 3))
      for j in range(N_skip, N_hist):
        name = file_prefix + '.' + str(i) + '.' + str(second_index) + '.base.gr.bodies.' + str(j) + '.dat'
        gr = sa.radial_distribution_function(x[j * N_avg : (j+1) * N_avg], N_avg, rcut=wall_Lz / 2, nbins=100, r_vectors=None, L=L, dim='q2d', name=name)      
        gr_mean[:,0] = gr[:,0]
        gr_mean[:,2] += (j - N_skip) * (gr[:,1] - gr_mean[:,1])**2 / (j - N_skip + 1)
        gr_mean[:,1] += (gr[:,1] - gr_mean[:,1]) / (j - N_skip + 1)
      gr_mean[:,2] = np.sqrt(gr_mean[:,2] / np.maximum(1, N_hist - N_skip) / np.maximum(1, N_hist - N_skip - 1))
      name = file_prefix + '.' + str(i) + '.' + str(second_index) + '.base.gr.bodies.dat'
      np.savetxt(name, gr_mean, header='Column: r, g(r), error g(r)')
